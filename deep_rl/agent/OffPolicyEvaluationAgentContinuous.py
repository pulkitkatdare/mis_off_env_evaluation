...
class OffPolicyEvaluationContinuous(BaseAgent):
    def __init__(self, config):
        ...
        self.replay_eval = config.replay_fn()
        self.total_steps = 0
    ...
    def collect_data(self):
        ...
        self.sample_trajectory_eval(self.noise_std)
    ...
    def sample_trajectory(self, std):
        ...
        for i in range(100):
            ...
            for j in range(150):
                ...
                next_states, rewards, done, info = self.env_p.step(action)
    ...
    def sample_trajectory_eval(self, std):
        config = self.config
        for i in range(100):
            print('Sampling trajectory %s' % (i))
            ...
            for j in range(150):
                ...
                experiences = list(zip(input_states, input_actions, input_rewards, input_next_states, input_done))
                self.replay_eval.feed_batch(experiences)
                states = next_states
    ...
    def compute_oracle(self):
        ...
        if config.game in ['CartPole-v1', 'Acrobot-v1']:
            n_ep = 100
        elif config.game in ['RoboschoolHalfCheetah-v1']:
            n_ep = 100
        else:
    ...
    def eval_episodes(self):
        if self.with_beta:
            experiences = self.replay_eval.sample(1000)#len(self.replay.data))
        else:
