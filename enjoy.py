...
def main():  # noqa: C901
    ...
    try:
        data = []
        for time in range(args.n_timesteps):
            old_obs = obs
            ...
            #print (np.shape(obs))
            action += np.random.normal(size=(1, 6))*1e-4
            
            #if args.env == 'RoboschoolHalfCheetah-v1':
                #print ("it comes here")
            action += np.random.normal(size=(1, 6))*args.policy_noise

            #if np.random.rand() < args.policy_noise:
            #    action = [np.random.randint(3)]
            obs, reward, done, infos = env.step(action)
            #print (np.shape(obs))
            data.append((old_obs, action, obs, reward, done))
            episode_start = done

            if not args.no_render:
                env.render("human")

            episode_reward += ((0.99)**ep_len)*reward[0]
            ep_len += 1
            ...
            if args.n_envs == 1:
                ...
                if done or ep_len == args.timesteps:
